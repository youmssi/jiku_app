...
object SeatAllocator {
    ...
    fun isOpen(current: Long, limit: Long?): Boolean = limit == null || current < limit
     ...
     */
    fun take(current: Long, limit: Long?): Seat =
        if (isOpen(current, limit)) Seat(taken = true, nextCount = current + 1)
        else Seat(taken = false, nextCount = current)
