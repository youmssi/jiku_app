...
import com.jiku.catalog.internal.ServiceUpdateRequest
...
class ServiceAdminTest {
    ...
    fun `create, list, rename and require resources for a service`() {
        ...
        val created = services.create(ServiceCreateRequest(name = "Coupe femme", timezone = "Africa/Conakry"))
        assertEquals("Coupe femme", created.name)
        assertTrue(services.list().any { it.id == created.id })

        assertEquals("Coupe homme", services.update(created.id, ServiceUpdateRequest(name = "Coupe homme")).name)
    ...
    fun `an invalid timezone is refused`() {
        ...
        val ex =
            assertThrows(ResponseStatusException::class.java) {
                services.create(ServiceCreateRequest(name = "Mauvaise zone", timezone = "Not/AZone"))
            }
    ...
    fun `another tenant never sees or edits a service`() {
        ...
        val service = services.create(ServiceCreateRequest(name = "Cabine A", timezone = "Africa/Conakry"))
